...
(function () {
  ...
  function isTargetText(text) {
    ...
    return !!t && t.length <= MAX_TEXT && /veo/.test(t) && TARGET_WORDS.test(t);
  }
  ...
  function clickLikeUser(el) {
    ...
    for (const type of ["pointerover", "pointerenter", "pointerdown", "mousedown", "pointerup", "mouseup", "click"]) {
      try { el.dispatchEvent(new MouseEvent(type, { bubbles: true, cancelable: true, composed: true, view: window, clientX: cx, clientY: cy, button: 0 })); } catch {}
    }
    try {
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      [data-dx-model-label="1"] { position:relative!important;overflow:hidden!important;color:transparent!important;text-shadow:none!important;-webkit-text-fill-color:transparent!important }
      [data-dx-model-label="1"] * { opacity:0!important;color:transparent!important;text-shadow:none!important;-webkit-text-fill-color:transparent!important }
      [data-dx-model-label="1"]::after { content:"${DISPLAY_LABEL}";position:absolute!important;inset:0!important;z-index:2147483647!important;display:flex!important;align-items:center!important;justify-content:flex-start!important;box-sizing:border-box!important;padding:0 14px!important;color:#f8fafc!important;-webkit-text-fill-color:#f8fafc!important;font:inherit!important;font-weight:600!important;letter-spacing:0!important;line-height:1!important;white-space:nowrap!important;pointer-events:none!important;opacity:1!important;text-shadow:0 0 10px rgba(255,255,255,.18)!important }
      [data-dx-model-trigger="1"] { background:rgba(34,34,34,.96)!important;border-color:rgba(255,255,255,.08)!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.05)!important }
      [data-dx-model-option="target"] { display:flex!important;min-height:42px!important;background:rgba(39,39,42,.98)!important }
      [data-dx-model-option="hidden"] { display:none!important;height:0!important;min-height:0!important;max-height:0!important;padding:0!important;margin:0!important;border:0!important;opacity:0!important;visibility:hidden!important;pointer-events:none!important }
    `;
  ...
  function candidateTriggers() {
    return [...document.querySelectorAll('button[aria-haspopup="listbox"], button[aria-haspopup="menu"], [role="combobox"], button')]
  ...
  function optionCandidates() {
    const nodes = [...document.querySelectorAll('[role="option"], [role="menuitem"], [data-radix-collection-item], li, button')];
    const seen = new Set();
    const out = [];
  ...
  function relabelAnyTargetText() {
    const walker = document.createTreeWalker(document.body || document.documentElement, NodeFilter.SHOW_TEXT, {
      acceptNode(node) {
        ...
        return t && t.length <= MAX_TEXT && isTargetText(t) ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_REJECT;
      },
    });
    ...
    while ((node = walker.nextNode())) {
      ...
      if (!p || parents.has(p) || p.closest('[role="option"], [role="menuitem"], [role="combobox"], [aria-haspopup]')) continue;
      parents.add(p);
    }
  ...
  async function forceSelectLite() {
    ...
    if (isTargetText(textOf(trigger))) { lockVisibleLabels(); return; }
    selecting = true;
    try {
